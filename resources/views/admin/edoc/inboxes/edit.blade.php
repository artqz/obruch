...
@section('content')
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item">
            <a class="nav-link active" data-toggle="tab" href="#card" role="tab" aria-controls="card" aria-selected="true">Карточка</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" data-toggle="tab" href="#uploads" role="tab" aria-controls="uploads" aria-selected="false">Файлы</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" data-toggle="tab" href="#users" role="tab" aria-controls="users" aria-selected="false">Исполнители</a>
        </li>
    </ul>
    <div class="tab-content" id="myTabContent">
        ...
        <div class="tab-pane fade" id="uploads" role="tabpanel" aria-labelledby="profile-tab">
            <br>
            {{ Form::open(['url' => 'admin/edoc/inbox/'.$inbox->id.'/upload', 'method' => 'post']) }}
                <div class="form-group">
                    {{ Form::label('file', 'Файл') }}
                    {{ Form::file('file', ['placeholder' => '1', 'class' => 'form-control']) }}
                    <small id="fileHelp" class="form-text text-muted">Выберите прикрепляемый файл и нажмите кнопку загрузить.</small>
                    <small id="fileHelp" class="text-danger">{{ $errors->first('file') }}</small>
                </div>
                {{ Form::submit('Загрузить изображение', ['class' => 'btn btn-success']) }}
            {{ Form::close() }}
        </div>
    </div>
@endsection
